...
class NASAdvanced(NASModel):
    def __init__(self, model_name, num_classes, input_dim, dataset, reshape_type,
        ...
        super(NASAdvanced, self).__init__(model_name=model_name, num_classes=num_classes, input_dim=input_dim,
                                          dataset=dataset, loss_function=loss_function,
                                          loss_function_name=loss_function_name, model_directory="nas_advanced",
    ...
    def _construct_model_(self):
        ...
        # Broaden search space: Allow up to 8 blocks and 5 layers per block for deeper temporal feature extraction
        x = ConvBlockExt(num_blocks=hp.Choice(name='num_blocks', values=[1, 2, 3, 4, 5, 6, 7, 8]),
                         num_layers=hp.Choice(name='num_layers', values=[1, 2, 3, 4, 5]),
                         filters=hp.Choice("filters", self.n_filters),
                         kernel_size=hp.Choice("kernel_size", self.kernel_size))(input_node)

        # Classification Block
        x = ak.Flatten()(x)
        x = DenseBlockExt(num_layers=hp.Choice(name='num_layers', values=[1, 2, 3, 4]),
